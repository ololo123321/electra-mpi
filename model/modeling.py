...
class BertModel(object):
    ...
    def __init__(self,
        ...
        embedding_scope = (scope if untied_embeddings else "electra") + "/embeddings"
        ...
        with tf.variable_scope(scope, default_name="electra"):
            ...
            with tf.variable_scope("encoder"):
                ...
                attention_mask = create_attention_mask_from_input_mask(token_type_ids, input_mask, use_fp16)
                ...
                self.all_layer_outputs, self.attn_maps = transformer_model(
                    input_tensor=tf.saturate_cast(self.embedding_output, infer_dtype(use_fp16)),
...
def gelu(x):
    ...
    """
    # cdf = 0.5 * (1.0 + tf.math.erf(x / tf.sqrt(2.0)))  # tf.math.erf не хавает tf.float16
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint, prefix=""):
    ...
    # name_to_variable['global_step'] = tf.train.get_or_create_global_step()
...
def embedding_postprocessor(input_tensor,
    ...
    if use_position_embeddings:
        ...
        with tf.control_dependencies([assert_op]):
            ...
            # the batch size.
            position_broadcast_shape = [1] * (num_dims - 2) + [seq_length, width]
